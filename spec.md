...
<a name="method-streaming"></a>streaming | `boolean` | Indicates that this method MAY return multiple results for a single request. When `true`, each result MUST share the same `id` as the original request. Default value is `false`.
